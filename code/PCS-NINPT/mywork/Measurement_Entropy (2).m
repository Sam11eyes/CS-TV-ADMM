...
function H = Measurement_Entropy(y, total_pixels)   %   计算熵值，编码

% assumed that y is vector.
y = y(:);

nbin = unique(y);                                      %     量化的电平数
frequency = hist(y,nbin);                          %     统计各个符号出现的概率      

p = frequency/sum(frequency);                %    计算概率
p(p==0) = 1;
H = sum(-p.*log2(p));                                %    测量值的信息熵（每个测量值编码需要的平均码长）
 
% total pixel is used to calculate the real entropy
H = H*numel(y)/total_pixels;                    %    实际熵值  
