...
function reconstructed_image = ...
    PCS_PL_ED_Decoder_mod (y,  initial, Phi, key_matrix, num_rows, num_cols, num_levels, max_iterations)
...
x = mod_encrypting  (initial, key_matrix, num_rows);
...
for i = 1:max_iterations
    ...
    i
  [x, D] = SPLIteration(y, x, Phi, pinvPhi, key_matrix, num_rows, num_cols, ...
      lambda, num_levels);
...
reconstructed_image = mod_decrypting (x, key_matrix, num_rows);
...
function [x, D] = SPLIteration(y, x, Phi, pinvPhi, key_matrix, num_rows, num_cols, ...
    lambda, num_levels, last)
...
x_hat = mod_decrypting (x_hat, key_matrix, num_rows);

%  2: TV投影

x_hat = wiener2(x_hat, [15, 15]);
...
x_bar = mod_encrypting  (x_bar, key_matrix, num_rows);
